...
import mail.storage.PersistentMailStorage;
...
public class MailManager {
	...
	synchronized (mail) {
	    CopyOnWriteArrayList<MailItem> mails = new CopyOnWriteArrayList<>(
		    mail);
	...
	synchronized (mail) {
	    CopyOnWriteArrayList<MailItem> items = new CopyOnWriteArrayList<>(
		    mail);
	    for (MailItem item : items) {
		PersistentMailStorage.storeMail(item.getRecipient(), item);
	    }
	}
    }
